...
def generate(priority,outlayer,inlayer,key_type,boundary):
    ...
    if drty_out == "M":
        ...
        over_out = gdstk.boolean(rect,gdstk.Reference(pre),"not", layer=int(outlayer))
        result.add(*over_out)
    else:
        rect = gdstk.rectangle((0,0),(int(bar_xy),int(bar_xy)))
        over_out = gdstk.boolean(rect,gdstk.Reference(pre),"and", layer=int(outlayer))
        result.add(*over_out)  
    if drty_in == "M":
        rect = gdstk.rectangle((0,0),(int(bar_xy),int(bar_xy)))
        over_in = gdstk.boolean(rect,gdstk.Reference(pre),"not", layer=int(inlayer))
        result.add(*over_in)
    else:
        rect = gdstk.rectangle((0,0),(int(bar_xy),int(bar_xy)))
        over_in = gdstk.boolean(rect,gdstk.Reference(pre),"and",layer=int(inlayer))
        result.add(*over_in)
...
if __name__ == "__main__":
    ...
    outlayer = 77
    inlayer = 96
    key_type = "OVERLAY_KY_T_1_0_NB_T_8_0_27_7"
