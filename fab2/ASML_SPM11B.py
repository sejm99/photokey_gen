...
def generate(priority,layer_no,key_type,boundary):
    ...
    ## SPM11B pattern drawing
    if key_name == "SPM11" and int(main_y) == 74 and float(bar_x) == 8.0:
       y1 = 1
       for i in range(0,40):
        if i < 21:
            x1 = 36+((8+float(bar_x))*i)
            spm11_s = gdstk.rectangle((x1,y1),(x1+float(bar_x),y1+float(bar_y)), layer=int(layer_no))
            pre.add(spm11_s)
        else:
            x1 = int(main_x)-(37.4+((9.6+float(bar_x)))*(i-21))
            spm11_s = gdstk.rectangle((x1,y1),(x1-float(bar_x),y1+float(bar_y)), layer=int(layer_no))
            pre.add(spm11_s)
        cross = gdstk.cross((400,37),5,1, layer=int(layer_no))
        pre.add(cross)
    elif key_name == "SPM11" and int(main_y) == 54 and float(bar_x) == 8.0:
       y1 = 2.5
       for i in range(0,40):
        if i < 21:
            x1 = 36+((8+float(bar_x))*i)
        ...
        pre.add(cross)
    elif key_name == "SPM11B" and int(main_y) == 74 and float(bar_x) == 8.0:
       y1 = 1
       for i in range(0,40):
        if i < 21:
            x1 = 36+((8+float(bar_x))*i)
            ...
            bar_x = 8.8
            x1 = int(main_x)-(37+((8.8+float(bar_x)))*(i-21))
            spm11_s = gdstk.rectangle((x1,y1),(x1-float(bar_x),y1+float(bar_y)), layer=int(layer_no))
            pre.add(spm11_s)
        cross = gdstk.cross((400,37),5,1, layer=int(layer_no))
        pre.add(cross)
    elif key_name == "SPM11B" and int(main_y) == 54 and float(bar_x) == 8.0:
       y1 = 2.5
       for i in range(0,40):
        if i < 21:
            x1 = 36+((8+float(bar_x))*i)
    ...
    if etch_type == "T":
        ...
        spm11 = gdstk.boolean(rect,gdstk.Reference(pre),"not",layer=int(layer_no))
        result.add(*spm11)
    else:
        rect = gdstk.rectangle((0,0),(int(main_x),int(main_y)))
        spm11 = gdstk.boolean(rect,gdstk.Reference(pre),"and",layer=int(layer_no))
        result.add(*spm11)
...
if __name__ == "__main__":
    priority = 1
    layer_no = 0
    key_type = "SPM11B_800_54_8_0_49_0_T"
