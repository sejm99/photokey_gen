...
def generate(priority,outlayer,inlayer,key_type,boundary,text):
   ...
   # OVERLAY_In/Out_Layer_TEXT_Define
    ...
    if txy_f == 'K':
        ...
        rect = gdstk.rectangle((0,0),((float(bar_xy),10)))
        if drty_in == "T":
        ...
        try:
            ...
            l_1.add(polygons)
        pre_ll.add(gdstk.Reference(l_1,(float(ul_1),float(txy_half))))
        pre_lr.add(gdstk.Reference(l_1,(float(ul_1),float(txy_half))))
        polygons = F2_MAKE_LETTER.main("L",int(inlayer),txy,txy)
        try:
            l_2.add(*polygons)
        except:
            l_2.add(polygons)
        pre_ll.add(gdstk.Reference(l_2,(float(ul_1)+float(txy)+2,float(txy_half))))
        pre_ul.add(gdstk.Reference(l_2,(float(ul_1)+float(txy)+2,float(txy_half))))
        polygons = F2_MAKE_LETTER.main("U",int(inlayer),txy,txy)
        try:
            u_1.add(*polygons)
        except:
            u_1.add(polygons)
        pre_ul.add(gdstk.Reference(u_1,(float(ul_1),float(txy_half))))
        pre_ur.add(gdstk.Reference(u_1,(float(ul_1),float(txy_half))))
        polygons = F2_MAKE_LETTER.main("R",int(inlayer),txy,txy)
        try:
            r_2.add(*polygons)
        except:
            r_2.add(polygons)
        pre_lr.add(gdstk.Reference(r_2,(float(ul_1)+float(txy)+2,float(txy_half))))
        pre_ur.add(gdstk.Reference(r_2,(float(ul_1)+float(txy)+2,float(txy_half))))
        ...
        bb_1 =((float(bar_xy))-((float(txy)*2)+2))/22
        ul_1 =((float(bar_xy))-((float(txy)*2)+2))/2
        txy_half = (10-float(txy))/2
        ...
        if txy_layer !='0':
            ...
            for i in range(0,len(txy_lay)):
                left.add(gdstk.rectangle((0,0),(float(bar_xy),10),layer=int(txy_lay[i])))
                right.add(gdstk.rectangle((0,0),(float(bar_xy),10),layer=int(txy_lay[i])))
                box.add(gdstk.rectangle((xy,xy),(xy+12,xy+12),layer=int(txy_lay[i])))
                ll_p.add(gdstk.rectangle((0,0),(float(bar_xy),10),layer=int(txy_lay[i])))
                lr_p.add(gdstk.rectangle((0,0),(float(bar_xy),10),layer=int(txy_lay[i])))
                ul_p.add(gdstk.rectangle((0,0),(float(bar_xy),10),layer=int(txy_lay[i])))
                ur_p.add(gdstk.rectangle((0,0),(float(bar_xy),10),layer=int(txy_lay[i]))) 
        else:
            pass

        # OVERLAY_Imlayer_TEXT_Define   ## letter,layer_no,size_x,size_y,thin
    ...
    for i in range(0,len(str(outlayer))):
        if outlayer == int(3) :
            text1 = gdstk.rectangle((0,0),((float(bar_xy),10)), layer=int(outlayer))
            left.add(text1)
            text2 = gdstk.rectangle((0,0),((float(bar_xy),10)),layer=int(outlayer))
            right.add(text2) 
            text3 = gdstk.rectangle((0,0),((float(bar_xy),10)),layer=int(outlayer))
            ll_p.add(text3)
            lr_p.add(text3)
            ul_p.add(text3)
            ur_p.add(text3)
        ...
        for i in range(0,len(boundary)):
            ...
            left.add(bound2)
            bound3 = gdstk.rectangle((0,0),((float(bar_xy),10)),layer=int(boundary[i]))
            right.add(bound3) 
            bound4 = gdstk.rectangle((0,0),((float(bar_xy),10)),layer=int(boundary[i]))
            ll_p.add(bound4)
            lr_p.add(bound4)
            ul_p.add(bound4)
            ur_p.add(bound4)
...
def start(priority,outlayer,inlayer,key_type,boundary,text,lllib,lrlib,ullib,urlib):
    ...
    bar_xy=(key_type.split('_')[9])          
    ll.add(gdstk.Reference(box,(0,20)))
    ll.add(gdstk.Reference(left,(0,10)))
    ll.add(gdstk.Reference(right,(0,0)))
    ll.add(gdstk.Reference(ll_p,(0,float(bar_xy)+20)))

    lr.add(gdstk.Reference(box,(0,20)))
    lr.add(gdstk.Reference(left,(0,10)))
    lr.add(gdstk.Reference(right,(0,0)))
    lr.add(gdstk.Reference(lr_p,(0,float(bar_xy)+20)))
                      
    ul.add(gdstk.Reference(box,(0,20)))
    ul.add(gdstk.Reference(left,(0,10)))
    ul.add(gdstk.Reference(right,(0,0)))
    ul.add(gdstk.Reference(ul_p,(0,float(bar_xy)+20)))
                        
    ur.add(gdstk.Reference(box,(0,20)))
    ur.add(gdstk.Reference(left,(0,10)))
    ur.add(gdstk.Reference(right,(0,0)))
    ur.add(gdstk.Reference(ur_p,(0,float(bar_xy)+20)))
