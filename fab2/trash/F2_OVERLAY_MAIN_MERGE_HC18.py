...
from fab2 import F2_OVERLAY_BL_BR_LETTER, F2_OVERLAY_IN_OUT_1ST
import pandas as pd
...
def main(process):
    ...
    if (OVERTYPE == '1' or OVERTYPE == '2' or OVERTYPE =='100' ) and os.path.isfile(OVERLAY_01) == True:
    ...
    if os.path.isfile('./fab2/text/1st_overlay.txt'):
        inlib = gdstk.Library() # lib 생성
        outlib = gdstk.Library()
        in_top_cell = process_spd + "_1ST_IN_" + date
        out_top_cell = process_spd + "_1ST_OUT_" + date
        in_top = inlib.new_cell(in_top_cell)
        out_top = outlib.new_cell(out_top_cell)
        for i in range(0,len(KEY_TYPE)):
            overlay_1st=KEY_TYPE[i]
            if overlay_1st.split('_')[0] == "1STOVER":
                ...
                pass
        output1_spd = './fab2/f2_gdsout/'+key_name_spd+'/'+in_top_cell+".gds"
        output2_spd = './fab2/f2_gdsout/'+key_name_spd+'/'+out_top_cell+".gds"
        inlib.write_gds(output1_spd)
        outlib.write_gds(output2_spd)
    else:
        pass
    gds_tar = glob.glob('./fab2/f2_gdsout/'+key_name_spd+'/'+"*1ST*.gds")
    for i in range(0,len(gds_tar)) :
        top1 = gds_tar[i].split('.gds')[0]
        top = os.path.basename(top1)
        frist_list = top.split('_')
        for i in range(0,len(frist_list)):
            if "1ST" in frist_list[i]:
                frsty = frist_list[i+1]
        out2 = './fab2/f2_gdsout/'+key_name_spd+'/'+key_name_spd+'/'+top+".1ST_"+frsty
        with open (out2,"w")as f:
            f.write("File_Type\t\t\t \"GDS\"\n")
            f.write("Hotspot_Name\t\t\t \"1\"\n")
            f.write("Hotspot_X\t\t\t \"0\"\n")
