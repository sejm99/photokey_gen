...
def generate(priority,layer_no,key_type,boundary):
    ...
    if float(bar_y) > 0.0 :
        if int(main_y) == 50 :
            ...
            # verti array-2
            bar_y1 = 34
            x2 = 14.5-(float(bar_x)/2)
            y2 = 25-(float(bar_y1)/2)
            for i in range(0,2):
                box_a = gdstk.rectangle((x2,y2),(x2+float(bar_x),y2+float(bar_y1)), layer=int(layer_no))
                pre.add(box_a)
                x2 += 101
    else:
...
if __name__ == "__main__":
    priority = 1
    layer_no = 0
    key_type = "20P-4FA_130_50_4_0_30_0_M"
