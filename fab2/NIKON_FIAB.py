...
def generate(priority,layer_no,key_type,boundary):
    ...
    ## FIA_BSI pattern drawing
    if (float(bar_y) == 49.0 or float(bar_y) == 70.0) :
        if float(bar_x) == 0.2 or float(bar_x) == 0.3 :
            y1 = 2.5
            if float(bar_x) == 0.2 :
                bar_x = 1.0
                for i in range(0,9):
                    x1 = 24+((11+float(bar_x))*i)
                    fia_b = gdstk.rectangle((x1,y1),(x1+float(bar_x),y1+float(bar_y)), layer=int(layer_no))
                    pre.add(fia_b)
                    x2 = float(x1+float(bar_x)+1.5)
                    fia_b = gdstk.rectangle((x2,y1),(x2+float(bar_x),y1+float(bar_y)), layer=int(layer_no))
                    pre.add(fia_b)
                    x3 = float(x2+float(bar_x)+1.5)
                    fia_b = gdstk.rectangle((x3,y1),(x3+float(bar_x),y1+float(bar_y)), layer=int(layer_no))
                    pre.add(fia_b)
            elif float(bar_x) == 0.3 :
                bar_x = 2.0
                for i in range(0,9):
                    x1 = 24+((10+float(bar_x))*i)
                    fia_b = gdstk.rectangle((x1,y1),(x1+float(bar_x),y1+float(bar_y)), layer=int(layer_no))
                    pre.add(fia_b)
                    x2 = float(x1+float(bar_x)+2.0)
                    fia_b = gdstk.rectangle((x2,y1),(x2+float(bar_x),y1+float(bar_y)), layer=int(layer_no))
                    pre.add(fia_b)
        else:
            x1 = 27-(float(bar_x)/2)
...
if __name__ == "__main__":
    priority = 1
    layer_no = 0
    key_type = "FIAB_150_75_2_0_70_0_T"
