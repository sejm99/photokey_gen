...
from fab2 import ASML_DOT53B, ASML_SPM53B, ASML_VSPM157B, ASML_SPM11B
import pandas as pd

def main(process,priority,layer_no,key_type,boundary,file_n):
    ## DOT53B/SPM53B/SPM11B/VSPM157B
    fn=(str(file_n))
    #fn=(str(file_n[-2]))
    ...
    for i in range(0,len(df_sort)) :
        ...
        if KEY_TYPE.split('_')[0] =="DOT53B":
            res = ASML_DOT53B.generate(int(PRIORITY),int(LAYER_NO),KEY_TYPE,BOUNDARY)
            xlib.add(res)
            ylib.add(res) 
        elif KEY_TYPE.split('_')[0] =="SPM53B":
            res = ASML_SPM53B.generate(int(PRIORITY),int(LAYER_NO),KEY_TYPE,BOUNDARY)
            xlib.add(res)
            ylib.add(res) 
        elif KEY_TYPE.split('_')[0] =="VSPM157B":
            res = ASML_VSPM157B.generate(int(PRIORITY),int(LAYER_NO),KEY_TYPE,BOUNDARY)
            xlib.add(res)
            ylib.add(res) 
        elif KEY_TYPE.split('_')[0] =="SPM11B":
            res = ASML_SPM11B.generate(int(PRIORITY),int(LAYER_NO),KEY_TYPE,BOUNDARY)
            xlib.add(res)
            ylib.add(res)        
        ...
        if i == 0 :  ## 첫번째 셀 배치 0,0 부터
            pro_bak = pro_now
            xcell.add(gdstk.Reference(res,(main_x,main_y),rotation=numpy.pi))
            ycell.add(gdstk.Reference(res,(main_y,0),rotation=numpy.pi/2))
            ...
            if pro_bak == pro_now :
                ...
                pro_bak = pro_now
                rect_x = main_x + rect_x
                x_max = x_max + main_x
            xcell.add(gdstk.Reference(res,(rect_x+main_x,main_y),rotation=numpy.pi))
            ycell.add(gdstk.Reference(res,(main_y,rect_x),rotation=numpy.pi/2))      
...
if __name__ == '__main__':
    ...
    priority = [1,2,3,4,5]
    layer_no = [3,6,9,8,5]
    key_type = ["FIA_150_54_6_0_49_0_T","FIA_150_54_6_0_49_0_T","FIA_150_54_1_5_6_0_M","FIA_150_54_6_0_49_0_M","FIA_150_54_6_0_49_0_M"]
    boundary = [[121,63],[63],[63,121],[100,200],[23,81]]
