...
def generate(priority,layer_no,key_type,boundary):
    ...
    if float(bar_y) > 0.0 :
        if int(main_y) == 50 :
            ...
            # verti array-2
            bar_y1 = 34
            x3 = 12.5-(float(bar_x)/2)
            y3 = 25-(float(bar_y1)/2)
            for i in range(0,2):
                box_a = gdstk.rectangle((x3,y3),(x3+float(bar_x),y3+float(bar_y1)), layer=int(layer_no))
                pre.add(box_a)
                x3 += 101
            x4 = 16.5-(float(bar_x)/2)
            for i in range(0,2):
                box_a = gdstk.rectangle((x4,y3),(x4+float(bar_x),y3+float(bar_y1)), layer=int(layer_no))
                pre.add(box_a)
                x4 += 101
    else:
...
if __name__ == "__main__":
    priority = 1
    layer_no = 0
    key_type = "20P-4FDA_130_50_0_6_30_0_M"
