...
def main(process):
                ...
                if BASEPAD != '-':
                    for i in range(0,len(BASEPAD)):
                        ...
                        if no == in_lay:
                            ...
                            blb_t.add(gdstk.rectangle((float(bar_xy)-1,0),((float(bar_xy),txt_y)), layer=int(out_num)))
                            brb_t.add(gdstk.rectangle((float(bar_xy),0),(((float(bar_xy)+1),txt_y)), layer=int(out_num)))
                        else:
                            pass
            else:
                if pro_bak == pro_now:
    ...
    if os.path.isfile('./fab2/text/dicer.txt'):  
        ...
        if scheck != 0:
            ...
            pass
        '''# S8000_external_file
        if os.path.isfile('./fab2/f2_gdsout/'+key_name+'/'+scell_name+".gds"):
            gds_tar = glob.glob('./fab2/f2_gdsout/'+key_name+'/'+'*S8000*.gds')
            for i in range(0,len(gds_tar)) :
                    s_top1 = gds_tar[i].split('.gds')[0]
                    s_top = os.path.basename(s_top1)
