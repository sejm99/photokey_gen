...
def get_arr():
    #input 파일 정보 가져오기
    with open('./fab1/spm11_input','r') as f:
        lines = f.readlines()
        for line in lines:
            line_st=line.strip()
...
def generate(priority,layer_no,gds_name,boundary,etch_type,main_x,main_y,bar_x,bar_y,process):
    ...
    for i in range(0,40):
        if i < 21:
            x1 = 15+36+((8+float(bar_x))*i)
        else:
            x1 = 15+437.8+((9.6+float(bar_x))*(i-21))
        spm_11_1 = gdstk.rectangle((x1, 2.5),(x1+float(bar_x),2.5+float(bar_y)), layer=int(layer_no))
        pre_result.add(spm_11_1)

        spm11_cross1 = gdstk.rectangle((15+399.5, 22.5),(15+400.5, 27.5), layer=int(layer_no))
        spm11_cross2 = gdstk.rectangle((15+397.5, 24.5),(15+402.5, 25.5), layer=int(layer_no))
        pre_result.add(spm11_cross1)
        pre_result.add(spm11_cross2)
    if etch_type == "M":
...
def start():
    ...
    lib1 = gdstk.Library()
    cell1 = lib1.new_cell(PROCESS[0]+"_SPM11_1_1_50_"+time.strftime("%Y%m%d"))
    lib2 = gdstk.Library()
    cell2 = lib2.new_cell(PROCESS[0]+"_SPM11_1_2_50_"+time.strftime("%Y%m%d"))
    lib3 = gdstk.Library()
    cell3 = lib3.new_cell(PROCESS[0]+"_SPM11_1_3_50_"+time.strftime("%Y%m%d"))
    lib4 = gdstk.Library()
    cell4 = lib4.new_cell(PROCESS[0]+"_SPM11_2_1_50_"+time.strftime("%Y%m%d"))
    lib5 = gdstk.Library()
    cell5 = lib5.new_cell(PROCESS[0]+"_SPM11_2_2_50_"+time.strftime("%Y%m%d"))
    lib6 = gdstk.Library()
    cell6 = lib6.new_cell(PROCESS[0]+"_SPM11_2_3_50_"+time.strftime("%Y%m%d"))
    lib7 = gdstk.Library()
    cell7 = lib7.new_cell(PROCESS[0]+"_SPM11_2_4_50_"+time.strftime("%Y%m%d"))
    lib8 = gdstk.Library()
    cell8 = lib8.new_cell(PROCESS[0]+"_SPM11_2_5_50_"+time.strftime("%Y%m%d"))
    lib9 = gdstk.Library()
    cell9 = lib9.new_cell(PROCESS[0]+"_SPM11_2_6_50_"+time.strftime("%Y%m%d"))
    for i in range(0,len(PRORITY)):
        res = ""
        res1 = ""
    ...
    if cell1.references != [] :
        lib1.write_gds("./fab1/f1_gdsout/pre_spm11_1.gds")
    if cell2.references != [] :
        lib2.write_gds("./fab1/f1_gdsout/pre_spm11_2.gds")
    if cell3.references != [] :
        lib3.write_gds("./fab1/f1_gdsout/pre_spm11_3.gds")
    if cell4.references != [] :
        lib4.write_gds("./fab1/f1_gdsout/pre_spm11_4.gds")
    if cell5.references != [] :
        lib5.write_gds("./fab1/f1_gdsout/pre_spm11_5.gds")
    if cell6.references != [] :
        lib6.write_gds("./fab1/f1_gdsout/pre_spm11_6.gds")
    if cell7.references != [] :
        lib7.write_gds("./fab1/f1_gdsout/pre_spm11_7.gds")
    if cell8.references != [] :
        lib8.write_gds("./fab1/f1_gdsout/pre_spm11_8.gds")
    if cell9.references != [] :
        lib9.write_gds("./fab1/f1_gdsout/pre_spm11_9.gds")
