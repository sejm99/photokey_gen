...
def get_arr():
    #input 파일 정보 가져오기
    with open('./fab1/fia_s2_input','r') as f:
        lines = f.readlines()
        for line in lines:
            line_st=line.strip()
...
def generate(priority,layer_no,gds_name,boundary,etch_type,main_x,main_y,bar_x,bar_y,process):
    ...
    #Main
    for i in range(0,6):
        y1 = 2+(float(bar_y)+2)*i
        for j in range(0,9):
            if float(bar_x) == 2:
                x1 = 15+26+(float(bar_x)+10)*j
                fia_s2_1 = gdstk.rectangle((x1, y1),(x1+float(bar_x),y1+float(bar_y)), layer=int(layer_no))
                pre_result.add(fia_s2_1)
            elif float(bar_x) == 4:
                x1 = 15+25+(float(bar_x)+8)*j
                fia_s2_1 = gdstk.rectangle((x1, y1),(x1+float(bar_x),y1+float(bar_y)), layer=int(layer_no))
                pre_result.add(fia_s2_1)
            elif float(bar_x) == 6:
                x1 = 15+24+(float(bar_x)+6)*j
                fia_s2_1 = gdstk.rectangle((x1, y1),(x1+float(bar_x),y1+float(bar_y)), layer=int(layer_no))
                pre_result.add(fia_s2_1)
    if etch_type == "M":
        rect = gdstk.rectangle((15, 0),(int(main_x)+15,int(main_y)))
        fia_s2_2 = gdstk.boolean(rect,gdstk.Reference(pre_result),"not",layer=int(layer_no))
        result.add(*fia_s2_2)
    else:
        rect = gdstk.rectangle((15, 0),(int(main_x)+15,int(main_y)))
        fia_s2_2 = gdstk.boolean(rect,gdstk.Reference(pre_result),"and",layer=int(layer_no))
        result.add(*fia_s2_2)
    return result
...
def start():
    ...
    lib = gdstk.Library()
    cell = lib.new_cell(PROCESS[0]+"_FIA_S2_50_"+time.strftime("%Y%m%d"))
    for i in range(0,len(PRORITY)):
    ...
    lib.write_gds("./fab1/f1_gdsout/FIA_S2.gds")
