...
def get_arr():
    #input 파일 정보 가져오기
    with open('./fab1/wga_x_input','r') as f:
        lines = f.readlines()
        for line in lines:
            line_st=line.strip()
...
def generate(priority,layer_no,gds_name,boundary,etch_type,main_x,main_y,bar_x,bar_y,process):
    ...
    for i in range(0,len(boundary_sp)):
        if process == "0600SJ00BB" or process == "0650SJ00BD":
            if boundary_sp[i] == "11" or boundary_sp[i] == "17":
                bound_1 = gdstk.rectangle((0, 15),(int(main_x),int(main_y)+15+50), layer=int(boundary_sp[i]))
                result.add(bound_1)
            else:
                bound_1 = gdstk.rectangle((0, 15),(int(main_x),int(main_y)+15), layer=int(boundary_sp[i]))
                result.add(bound_1)
    ...
    for i in range(0,3):
        if i == 0:
            ...
            x1=41-float(bar_x)/2
        if main_y == "76":
            if float(bar_x) == 6 and float(bar_y) == 6:
                for j in range(0,5):
                    y1 = 15+11+((float(bar_y)*j)+(6*j))
                    wga_x1 = gdstk.rectangle((x1, y1),(x1+float(bar_x),y1+float(bar_y)), layer=int(layer_no))
                    pre_result.add(wga_x1)
            else:
                for j in range(0,7):
                    if float(bar_x) == 4:
                        y1 = 15+12+((float(bar_y)*j)+(4*j))
                        wga_x1 = gdstk.rectangle((x1, y1),(x1+float(bar_x),y1+float(bar_y)), layer=int(layer_no))
                        pre_result.add(wga_x1)
                    else:
                        y1 = 15+11.7+((float(bar_y)*j)+(3.4*j))
                        wga_x1 = gdstk.rectangle((x1, y1),(x1+float(bar_x),y1+float(bar_y)), layer=int(layer_no))
                        pre_result.add(wga_x1)
        elif main_y == "86":
            for j in range(0,7):
                if float(bar_x) == 5:
                    y1 = 15+10.5+((float(bar_y)*j)+(5*j))
                    wga_x1 = gdstk.rectangle((x1, y1),(x1+float(bar_x),y1+float(bar_y)), layer=int(layer_no))
                    pre_result.add(wga_x1)
                else:
                    y1 = 15+10.2+(float(bar_y)*j)+(4.4*j)
                    wga_x1 = gdstk.rectangle((x1, y1),(x1+float(bar_x),y1+float(bar_y)), layer=int(layer_no))
                    pre_result.add(wga_x1)
        else:
            for j in range(0,7):
                ...
                pre_result.add(wga_x1)  
    if etch_type == "M":
        rect = gdstk.rectangle((0, 15),(int(main_x),int(main_y)+15))
        wga_x2 = gdstk.boolean(rect,gdstk.Reference(pre_result),"not",layer=int(layer_no))
        result.add(*wga_x2)
    else:
        rect = gdstk.rectangle((0, 15),(int(main_x),int(main_y)+15))
        wga_x2 = gdstk.boolean(rect,gdstk.Reference(pre_result),"and",layer=int(layer_no))
        result.add(*wga_x2)
    return result
...
def generate_name(priority,layer_no,gds_name,boundary,etch_type,main_x,main_y,bar_x,bar_y,process):
    ...
    try:
        ...
        let_1.add(polygons)
    pre_result.add(gdstk.Reference(let_1,((int(main_x)/4)-5,2.5)))
    polygons = Make_letter(Layer_name[1],int(layer_no),10,10)
    try:
        let_2.add(*polygons)
    except:
        let_2.add(polygons)
    pre_result.add(gdstk.Reference(let_2,(((int(main_x)/4)*3)-5,2.5)))  

    rect = gdstk.rectangle((0, 0),(int(main_x),15))
    if etch_type == "M":
    ...
    boundary_sp = boundary.split(",")
    for i in range(0,len(boundary_sp)):
        bound_1 = gdstk.rectangle((0, 0),(int(main_x),15), layer=int(boundary_sp[i]))
        result.add(bound_1)
    return result
...
def start():
    ...
    lib = gdstk.Library()
    cell = lib.new_cell(PROCESS[0]+"_WGAX_50_"+time.strftime("%Y%m%d"))
    for i in range(0,len(PRORITY)):
        ...
        for j in range(0,len(ARR_SIZE_Y)):
            if int(ARR_SIZE_Y[j].split("_")[0]) < int(PRORITY[i]):
                str_y = str_y + 15 + int(ARR_SIZE_Y[j].split("_")[-1]) + 50
            else:
                break
        cell.add(gdstk.Reference(res,(0,str_y)))
        res1 = main_name(PRORITY[i],LAYER_NO[i],GDS_NAME[i],PAD_NO[i],ETCH_TYPE[i],MAIN_X[i],MAIN_Y[i],BAR_X[i],BAR_Y[i],PROCESS[0]) 
        lib.add(res1)
        cell.add(gdstk.Reference(res1,(0,str_y)))
    lib.write_gds("./fab1/f1_gdsout/WGA_X.gds")
