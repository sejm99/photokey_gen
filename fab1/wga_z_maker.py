...
def get_arr():
    #input 파일 정보 가져오기
    with open('./fab1/wga_z_input','r') as f:
        lines = f.readlines()
        for line in lines:
            line_st=line.strip()
...
def generate(priority,layer_no,gds_name,boundary,etch_type,main_x,main_y,bar_x,bar_y,process):
    ...
    for i in range(0,3):
        if i == 0:
            ...
            y1=41-float(bar_y)/2
        if float(bar_x)==5 and float(bar_y)==6:
            for j in range(0,20):
                x1 = 15+2.5+((5+float(bar_x))*j)
                wga_z1 = gdstk.rectangle((x1, y1),(x1+float(bar_x),y1+float(bar_y)), layer=int(layer_no))
                pre_result.add(wga_z1)
        else:
            for j in range(0,33):
                if float(bar_x) == 3:
                    x1 = 15+2.5+((3+float(bar_x))*j)
                    wga_z1 = gdstk.rectangle((x1, y1),(x1+float(bar_x),y1+float(bar_y)), layer=int(layer_no))
                    pre_result.add(wga_z1)
                elif float(bar_x)==6 and float(bar_y)==6:
                    x1 = 15+2.5+((3+float(bar_x))*j)
                    wga_z1 = gdstk.rectangle((x1, y1),(x1+float(bar_x),y1+float(bar_y)), layer=int(layer_no))
                    pre_result.add(wga_z1)
                else:
                    x1 = 15+2.2+((2.4+float(bar_x))*j)
                    wga_z1 = gdstk.rectangle((x1, y1),(x1+float(bar_x),y1+float(bar_y)), layer=int(layer_no))
                    pre_result.add(wga_z1)
    if etch_type == "M":
        rect = gdstk.rectangle((15, 0),(int(main_x)+15,int(main_y)))
        wga_z2 = gdstk.boolean(rect,gdstk.Reference(pre_result),"not",layer=int(layer_no))
        result.add(*wga_z2)
    else:
        rect = gdstk.rectangle((15, 0),(int(main_x)+15,int(main_y)))
        wga_z2 = gdstk.boolean(rect,gdstk.Reference(pre_result),"and",layer=int(layer_no))
        result.add(*wga_z2)
    return result
...
def start():
    ...
    lib = gdstk.Library()
    cell = lib.new_cell(PROCESS[0]+"_WGAZ_50_"+time.strftime("%Y%m%d"))
    for i in range(0,len(PRORITY)):
    ...
    lib.write_gds("./fab1/f1_gdsout/WGA_Z.gds")
