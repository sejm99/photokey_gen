...
def get_arr():
    #input 파일 정보 가져오기
    with open('./fab1/fia_2d_input','r') as f:
        lines = f.readlines()
        for line in lines:
            line_st=line.strip()
...
def generate(priority,layer_no,gds_name,boundary,etch_type,main_x,main_y,bar_x,bar_y,process):
    ...
    for i in range(0,10):
        for j in range(0,6):
            if i==0 or i==1:
                x1 = 15+20+(float(bar_x)+8)*i
                y1 = 2+(float(bar_y)+2)*j
                fia_2d_1 = gdstk.rectangle((x1, y1),(x1+float(bar_x),y1+float(bar_y)), layer=int(layer_no))
                pre_result.add(fia_2d_1)
            elif i==8 or i==9:
                x1 = 15+92+(float(bar_x)+8)*(i-8)
                y1 = 2+(float(bar_y)+2)*j
                fia_2d_1 = gdstk.rectangle((x1, y1),(x1+float(bar_x),y1+float(bar_y)), layer=int(layer_no))
                pre_result.add(fia_2d_1)
            else:
                if j < 4:
                    x1 = 15+39+(float(bar_y)+2)*(i-2)
                    y1 = 9+(float(bar_x)+8)*j
                    fia_2d_1 = gdstk.rectangle((x1, y1),(x1+float(bar_y),y1+float(bar_x)), layer=int(layer_no))
                    pre_result.add(fia_2d_1)

    if etch_type == "M":
        rect = gdstk.rectangle((15, 0),(int(main_x)+15,int(main_y)))
        fia_2d_2 = gdstk.boolean(rect,gdstk.Reference(pre_result),"not",layer=int(layer_no))
        result.add(*fia_2d_2)
    else:
        rect = gdstk.rectangle((15, 0),(int(main_x)+15,int(main_y)))
        fia_2d_2 = gdstk.boolean(rect,gdstk.Reference(pre_result),"and",layer=int(layer_no))
        result.add(*fia_2d_2)
    return result
...
def start():
    ...
    lib = gdstk.Library()
    cell = lib.new_cell(PROCESS[0]+"_FIA_2D_50_"+time.strftime("%Y%m%d"))
    for i in range(0,len(PRORITY)):
    ...
    lib.write_gds("./fab1/f1_gdsout/FIA_2D.gds")
