...
def get_arr():
    #input 파일 정보 가져오기
    with open('./fab1/aa157_input','r') as f:
        lines = f.readlines()
        for line in lines:
            line_st=line.strip()
...
def generate(priority,layer_no,gds_name,boundary,etch_type,main_x,main_y,bar_x,bar_y,process):
    ...
    for i in range(0,120):
        if i < 9:
            x1 = 15+44.9+((8+8)*i)
            aa157_1 = gdstk.rectangle((x1, 2.5),(x1+8,2.5+45), layer=int(layer_no))
        else:
            if i < (9+43):
                x1 = 15+236.9+((1.6+1.6)*(i-9))
                aa157_1 = gdstk.rectangle((x1, 2.5),(x1+1.6,2.5+45), layer=int(layer_no))
            else:
                if i < (9+43+60):
                    if x2 == "":
                        x2 = 15+428.9
                        ...
                        if i <= 53 + (7*j):
                            if i == 53 + (7*j):
                                ...
                                for k in range(0,10):
                                    if i <= 57 + (7*k):
                                        if i == 57 + (7*k):
                                            aa157_1 = gdstk.rectangle((x1, 2.5),(x1+1.1,2.5+45), layer=int(layer_no))
                                            x2 = x1 + 1.1 + 1.1
                                            break
                                        else:
                                            aa157_1 = gdstk.rectangle((x1, 2.5),(x1+1.15,2.5+45), layer=int(layer_no))
                                            x2 = x1 + 1.15 + 1.1
                                            break
                                break
                            else:
                    ...
                    x1 = 15+623.1+((8.8+8.8)*(i-(9+43+60)))
                    aa157_1 = gdstk.rectangle((x1, 2.5),(x1+8.8,2.5+45), layer=int(layer_no))
        pre_result.add(aa157_1)

        cross1 = gdstk.rectangle((15+400.4, 22.5),(15+401.4, 27.5), layer=int(layer_no))
        cross2 = gdstk.rectangle((15+398.4, 24.5),(15+403.4, 25.5), layer=int(layer_no))
        pre_result.add(cross1)
        pre_result.add(cross2)
    if etch_type == "M":
        rect = gdstk.rectangle((15,0),(int(main_x)+15,int(main_y)))
        bound1 = gdstk.boolean(rect,gdstk.Reference(pre_result),"not",layer=int(layer_no))
        result.add(*bound1)
    else:
        rect = gdstk.rectangle((15, 0),(int(main_x)+15,int(main_y)))
        bound2 = gdstk.boolean(rect,gdstk.Reference(pre_result),"and",layer=int(layer_no))
        result.add(*bound2)
    return result
...
def start():
    ...
    lib1 = gdstk.Library()
    cell1 = lib1.new_cell(PROCESS[0]+"_AA157_1_1_50_"+time.strftime("%Y%m%d"))
    lib2 = gdstk.Library()
    cell2 = lib2.new_cell(PROCESS[0]+"_AA157_1_2_50_"+time.strftime("%Y%m%d"))
    lib3 = gdstk.Library()
    cell3 = lib3.new_cell(PROCESS[0]+"_AA157_1_3_50_"+time.strftime("%Y%m%d"))
    lib4 = gdstk.Library()
    cell4 = lib4.new_cell(PROCESS[0]+"_AA157_2_1_50_"+time.strftime("%Y%m%d"))
    lib5 = gdstk.Library()
    cell5 = lib5.new_cell(PROCESS[0]+"_AA157_2_2_50_"+time.strftime("%Y%m%d"))
    lib6 = gdstk.Library()
    cell6 = lib6.new_cell(PROCESS[0]+"_AA157_2_3_50_"+time.strftime("%Y%m%d"))
    lib7 = gdstk.Library()
    cell7 = lib7.new_cell(PROCESS[0]+"_AA157_2_4_50_"+time.strftime("%Y%m%d"))
    lib8 = gdstk.Library()
    cell8 = lib8.new_cell(PROCESS[0]+"_AA157_2_5_50_"+time.strftime("%Y%m%d"))
    lib9 = gdstk.Library()
    cell9 = lib9.new_cell(PROCESS[0]+"_AA157_2_6_50_"+time.strftime("%Y%m%d"))
    ...
    if cell1.references != [] :
        lib1.write_gds("./fab1/f1_gdsout/pre_AA157_1.gds")
    if cell2.references != [] :
        lib2.write_gds("./fab1/f1_gdsout/pre_AA157_2.gds")
    if cell3.references != [] :
        lib3.write_gds("./fab1/f1_gdsout/pre_AA157_3.gds")
    if cell4.references != [] :
        lib4.write_gds("./fab1/f1_gdsout/pre_AA157_4.gds")
    if cell5.references != [] :
        lib5.write_gds("./fab1/f1_gdsout/pre_AA157_5.gds")
    if cell6.references != [] :
        lib6.write_gds("./fab1/f1_gdsout/pre_AA157_6.gds")
    if cell7.references != [] :
        lib7.write_gds("./fab1/f1_gdsout/pre_AA157_7.gds")
    if cell8.references != [] :
        lib8.write_gds("./fab1/f1_gdsout/pre_AA157_8.gds")
    if cell9.references != [] :
        lib9.write_gds("./fab1/f1_gdsout/pre_AA157_9.gds")
