...
def get_arr():
    #input 파일 정보 가져오기
    with open('./fab1/lsa_input','r') as f:
        lines = f.readlines()
        for line in lines:
            line_st=line.strip()
...
def PRO_ARR_SIZE_SORT_Y():
    ...
    ARR_SIZE_PRE.sort(key=lambda x:(x[0],x[1]))
    return ARR_SIZE_PRE
def generate(priority,layer_no,gds_name,boundary,etch_type,main_x,main_y,bar_x,bar_y,arr_x,arr_y,process):
    ...
    for i in range(0,5):
        if float(arr_y) == 4:
            if float(bar_x) == 4:
                for j in range(0,4):
                    x1 = 15+33+(float(bar_x)+16)*i
                    y1 = 9.5+(float(bar_y)+5)*j
                    lsa_1 = gdstk.rectangle((x1, y1),(x1+float(bar_x),y1+float(bar_y)), layer=int(layer_no))
                    pre_result.add(lsa_1)
            else:
                for j in range(0,4):
                    x1 = 15+32+(float(bar_x)+14)*i
                    y1 = 4+(float(bar_y)+6)*j
                    lsa_1 = gdstk.rectangle((x1, y1),(x1+float(bar_x),y1+float(bar_y)), layer=int(layer_no))
                    pre_result.add(lsa_1)
        elif float(arr_y) == 6:
            for j in range(0,6):
                x1 = 15+33+(float(bar_x)+16)*i
                y1 = 3+(float(bar_y)+4)*j
                lsa_1 = gdstk.rectangle((x1, y1),(x1+float(bar_x),y1+float(bar_y)), layer=int(layer_no))
                pre_result.add(lsa_1)

    if etch_type == "M":
        rect = gdstk.rectangle((15, 0),(int(main_x)+15,int(main_y)))
        lsa_2 = gdstk.boolean(rect,gdstk.Reference(pre_result),"not",layer=int(layer_no))
        result.add(*lsa_2)
    else:
        rect = gdstk.rectangle((15, 0),(int(main_x)+15,int(main_y)))
        lsa_2 = gdstk.boolean(rect,gdstk.Reference(pre_result),"and",layer=int(layer_no))
        result.add(*lsa_2)
    return result
...
def main(priority,layer_no,gds_name,boundary,etch_type,main_x,main_y,bar_x,bar_y,arr_x,arr_y,process):
    res = generate(priority,layer_no,gds_name,boundary,etch_type,main_x,main_y,bar_x,bar_y,arr_x,arr_y,process)
    return res
...
def start():
    ...
    lib = gdstk.Library()
    cell = lib.new_cell(PROCESS[0]+"_LSA_50_"+time.strftime("%Y%m%d"))
    for i in range(0,len(PRORITY)):
        res = main(PRORITY[i],LAYER_NO[i],GDS_NAME[i],PAD_NO[i],ETCH_TYPE[i],MAIN_X[i],MAIN_Y[i],BAR_X[i],BAR_Y[i],ARR_X[i],ARR_Y[i],PROCESS[0]) 
        lib.add(res)
        str_x = 0
    ...
    lib.write_gds("./fab1/f1_gdsout/LSA.gds")
