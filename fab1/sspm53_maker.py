...
def get_arr():
    #input 파일 정보 가져오기
    with open('./fab1/sspm53_input','r') as f:
        lines = f.readlines()
        for line in lines:
            line_st=line.strip()
...
def generate(priority,layer_no,gds_name,boundary,etch_type,main_x,main_y,bar_x,bar_y,process):
    ...
    for i in range(0,17):
        if i < 9:
            x1 = 15+54.5+((14.4+float(bar_x))*i)
        else:
            x1 = 15+247.5+((16+float(bar_x))*(i-9))
        sspm_1 = gdstk.rectangle((x1, 2.5),(x1+float(bar_x),2.5+float(bar_y)), layer=int(layer_no))
        pre_result.add(sspm_1)
        x2 = float(x1+float(bar_x)+1.6)
        sspm_1 = gdstk.rectangle((x2, 2.5),(x2+float(bar_x),2.5+float(bar_y)), layer=int(layer_no))
        pre_result.add(sspm_1)
        x3 = float(x2+float(bar_x)+1.6)
        sspm_1 = gdstk.rectangle((x3, 2.5),(x3+float(bar_x),2.5+float(bar_y)), layer=int(layer_no))
        pre_result.add(sspm_1)    

        sspm_cross1 = gdstk.rectangle((15+218, 22.5),(15+219, 27.5), layer=int(layer_no))
        sspm_cross2 = gdstk.rectangle((15+216, 24.5),(15+221, 25.5), layer=int(layer_no))
        pre_result.add(sspm_cross1)
        pre_result.add(sspm_cross2)
    if etch_type == "M":
        rect = gdstk.rectangle((15,0),(int(main_x)+15,int(main_y)))
        sspm_2 = gdstk.boolean(rect,gdstk.Reference(pre_result),"not",layer=int(layer_no))
        result.add(*sspm_2)
    else:
        rect = gdstk.rectangle((15, 0),(int(main_x)+15,int(main_y)))
        sspm_2 = gdstk.boolean(rect,gdstk.Reference(pre_result),"and",layer=int(layer_no))
        result.add(*sspm_2)
    return result
...
def start():
    ...
    lib1 = gdstk.Library()
    cell1 = lib1.new_cell(PROCESS[0]+"_SSPM53_1_1_50_"+time.strftime("%Y%m%d"))
    lib2 = gdstk.Library()
    cell2 = lib2.new_cell(PROCESS[0]+"_SSPM53_1_2_50_"+time.strftime("%Y%m%d"))
    lib3 = gdstk.Library()
    cell3 = lib3.new_cell(PROCESS[0]+"_SSPM53_1_3_50_"+time.strftime("%Y%m%d"))
    lib4 = gdstk.Library()
    cell4 = lib4.new_cell(PROCESS[0]+"_SSPM53_2_1_50_"+time.strftime("%Y%m%d"))
    lib5 = gdstk.Library()
    cell5 = lib5.new_cell(PROCESS[0]+"_SSPM53_2_2_50_"+time.strftime("%Y%m%d"))
    lib6 = gdstk.Library()
    cell6 = lib6.new_cell(PROCESS[0]+"_SSPM53_2_3_50_"+time.strftime("%Y%m%d"))
    lib7 = gdstk.Library()
    cell7 = lib7.new_cell(PROCESS[0]+"_SSPM53_2_4_50_"+time.strftime("%Y%m%d"))
    lib8 = gdstk.Library()
    cell8 = lib8.new_cell(PROCESS[0]+"_SSPM53_2_5_50_"+time.strftime("%Y%m%d"))
    lib9 = gdstk.Library()
    cell9 = lib9.new_cell(PROCESS[0]+"_SSPM53_2_6_50_"+time.strftime("%Y%m%d"))
    ...
    if cell1.references != [] :
        lib1.write_gds("./fab1/f1_gdsout/pre_sspm53_1.gds")
    if cell2.references != [] :
        lib2.write_gds("./fab1/f1_gdsout/pre_sspm53_2.gds")
    if cell3.references != [] :
        lib3.write_gds("./fab1/f1_gdsout/pre_sspm53_3.gds")
    if cell4.references != [] :
        lib4.write_gds("./fab1/f1_gdsout/pre_sspm53_4.gds")
    if cell5.references != [] :
        lib5.write_gds("./fab1/f1_gdsout/pre_sspm53_5.gds")
    if cell6.references != [] :
        lib6.write_gds("./fab1/f1_gdsout/pre_sspm53_6.gds")
    if cell7.references != [] :
        lib7.write_gds("./fab1/f1_gdsout/pre_sspm53_7.gds")
    if cell8.references != [] :
        lib8.write_gds("./fab1/f1_gdsout/pre_sspm53_8.gds")
    if cell9.references != [] :
        lib9.write_gds("./fab1/f1_gdsout/pre_sspm53_9.gds")
