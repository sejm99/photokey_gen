...
def get_arr():
    #input 파일 정보 가져오기
    with open('./fab1/fia_input','r') as f:
        lines = f.readlines()
        for line in lines:
            line_st=line.strip()
...
def generate(priority,layer_no,gds_name,boundary,etch_type,main_x,main_y,bar_x,bar_y,process):
    ...
    for i in range(0,9):
        if float(bar_x)==6:
            x1 = 15+24+((6+float(bar_x))*i)
            fia_1 = gdstk.rectangle((x1, 2.5),(x1+float(bar_x),2.5+float(bar_y)), layer=int(layer_no))
            pre_result.add(fia_1)
        else:
            x1 = 15+23.7+((5.4+float(bar_x))*i)
            fia_1 = gdstk.rectangle((x1, 2.5),(x1+float(bar_x),2.5+float(bar_y)), layer=int(layer_no))
            pre_result.add(fia_1)
    if etch_type == "M":
        rect = gdstk.rectangle((15,0),(int(main_x)+15,int(main_y)))
        fia_2 = gdstk.boolean(rect,gdstk.Reference(pre_result),"not",layer=int(layer_no))
        result.add(*fia_2)
    else:
        rect = gdstk.rectangle((15, 0),(int(main_x)+15,int(main_y)))
        fia_2 = gdstk.boolean(rect,gdstk.Reference(pre_result),"and",layer=int(layer_no))
        result.add(*fia_2)
    return result
...
def start():
    ...
    lib = gdstk.Library()
    cell = lib.new_cell(PROCESS[0]+"_FIA_50_"+time.strftime("%Y%m%d"))
    for i in range(0,len(PRORITY)):
    ...
    lib.write_gds("./fab1/f1_gdsout/FIA.gds")
